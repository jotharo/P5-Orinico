...
const fetchTeddy = fetch (apiURL + id) 
    ...
    function addToCart(event) {
        ...
        // Variable - On créé un localStorage teddiesInCart ou un array vide si null 
        let cartTeddies = JSON.parse(localStorage.getItem('cartTeddies')) || [];

        let teddy = cartTeddies.find(item => {
            return item.teddyId === selectedTeddy.teddyId && item.tedydColor === selectedTeddy.teddyColor
        })
